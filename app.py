from fastapi import FastAPI, UploadFile, File, Form, Response, Query, Depends, HTTPException, status
...
security = HTTPBasic()
ADMIN_EMAIL = "raviram@ai.com"  # మీ ఈమెయిల్ ఐడి ఇక్కడ సెట్ చేసుకోవచ్చు
ADMIN_PASS = "raviram0536"

def verify_email_login(credentials: HTTPBasicCredentials = Depends(security)):
    # ఇక్కడ యూజర్ నేమ్ బదులుగా ఈమెయిల్ ఐడి తో లాగిన్ చెక్ చేయబడుతుంది
    if credentials.username != ADMIN_EMAIL or credentials.password != ADMIN_PASS:
        raise HTTPException(
...
async def edit_and_generate_video(
    ...
    user_email: str = Form("guest@gmail.com") # యూజర్ వారి ఈమెయిల్ ఐడిని ఇక్కడ ఇవ్వవచ్చు
):
    try:
        ...
        if secret_key == ADMIN_SECRET_KEY or user_email == ADMIN_EMAIL:
            role = "Boss (Raviram)"
            status = "Success (Free Access)"
            
            log_data = {
                "timestamp": current_time, "user_email": user_email, "role": role, "project_type": project_type,
            ...
            return {
                "status": "Success",
                "role": role,
                "message": f"Welcome Boss Raviram ({user_email})! Free access granted securely.",
        ...
        return {
            "status": status,
            "role": role,
            "message": f"Please pay ₹{amount} to process your video, {user_email}.",
